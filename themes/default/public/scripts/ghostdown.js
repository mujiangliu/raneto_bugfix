...
window.CodeMirror = (function () {
  ...
  function keyMapChanged (cm) {
    var map = keyMap[cm.options.keyMap]; var style = map.style;
  ...
  function updateGutters (cm) {
    var gutters = cm.display.gutters; var specs = cm.options.gutters;
  ...
  function updateScrollbars (cm) {
    var d = cm.display; var docHeight = cm.doc.height;
  ...
  function visibleLines (display, doc, viewPort) {
    var top = display.scroller.scrollTop; var height = display.wrapper.clientHeight;
  ...
  function alignHorizontally (cm) {
    ...
    var gutterW = display.gutters.offsetWidth; var l = comp + 'px';
  ...
  function updateDisplay (cm, changes, viewPort, forced) {
    var oldFrom = cm.display.showingFrom; var oldTo = cm.display.showingTo; var updated;
  ...
  function updateDisplayInner (cm, changes, visible, forced) {
    ...
    if (!forced && changes.length == 0 &&
        visible.from > display.showingFrom && visible.to < display.showingTo) { return; }
    ...
    if (cm.options.lineNumbers) {
      for (var i = 0; i < changes.length; ++i) { if (changes[i].diff) { positionsChangedFrom = changes[i].from; break; } }
    }
  ...
  function patchDisplay (cm, from, to, intact, updateNumbersFrom) {
    ...
    var nextIntact = intact.shift(); var lineN = from;
    cm.doc.iter(from, to, function (line) {
      ...
      if (lineIsHidden(cm.doc, line)) {
        ...
        if (line.widgets) {
          for (var j = 0, search = cur, reuse; search && j < 20; ++j, search = search.nextSibling) { if (search.lineObj == line && /div/i.test(search.nodeName)) { reuse = search; break; } }
        }
  ...
  function buildLineElement (cm, line, lineNo, dims, reuse) {
    ...
    var markers = line.gutterMarkers; var display = cm.display; var wrap;
    ...
    if (reuse) {
      ...
      var isOk = true; var widgetsSeen = 0; var insertBefore = null;
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (markers) {
        for (var k = 0; k < cm.options.gutters.length; ++k) {
          var id = cm.options.gutters[k]; var found = markers.hasOwnProperty(id) && markers[id];
          if (found) {
  ...
  function updateSelection (cm) {
    ...
    if (cm.options.moveInputWithCursor) {
      ...
      var wrapOff = getRect(display.wrapper); var lineOff = getRect(display.lineDiv);
  ...
  function updateSelectionCursor (cm) {
    var display = cm.display; var pos = cursorCoords(cm, cm.doc.sel.head, 'div');
  ...
  function updateSelectionRange (cm) {
    var display = cm.display; var doc = cm.doc; var sel = cm.doc.sel;
    var fragment = document.createDocumentFragment();
    var clientWidth = display.lineSpace.offsetWidth; var pl = paddingLeft(cm.display);
    ...
    function drawForLine (line, fromArg, toArg) {
      ...
      iterateBidiSections(getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg, function (from, to, dir) {
        var leftPos = coords(from, 'left'); var rightPos; var left; var right;
        if (from == to) {
          rightPos = leftPos;
    ...
    if (sel.from.line == sel.to.line) {
      ...
      var fromLine = getLine(doc, sel.from.line); var toLine = getLine(doc, sel.to.line);
  ...
  function getStateBefore (cm, n, precise) {
    ...
    var pos = findStartLine(cm, n, precise); var state = pos > doc.first && getLine(doc, pos - 1).stateAfter;
  ...
  function findCachedMeasurement (cm, line) {
    ...
    for (var i = 0; i < cache.length; ++i) {
      ...
      if (memo.text == line.text && memo.markedSpans == line.markedSpans &&
          ...
          memo.classes == line.textClass + '|' + line.bgClass + '|' + line.wrapClass) { return memo; }
    }
  }
  ...
  function measureLineInner (cm, line) {
    var display = cm.display; var measure = emptyArray(line.text.length);
    ...
    if (ie && !ie_lt8 && !cm.options.lineWrapping && pre.childNodes.length > 100) {
      var fragment = document.createDocumentFragment();
      var chunk = 10; var n = pre.childNodes.length;
    ...
    var vranges = []; var data = emptyArray(line.text.length); var maxBot = pre.offsetHeight;
    // Work around an IE7/8 bug where it will sometimes have randomly
    // replaced our pre with a clone at this point.
    if (ie_lt9 && display.measure.first != pre) { removeChildrenAndAdd(display.measure, pre); }

    function measureRect (rect) {
      var top = rect.top - outer.top; var bot = rect.bottom - outer.top;
      if (bot > maxBot) bot = maxBot;
      if (top < 0) top = 0;
      for (var i = vranges.length - 2; i >= 0; i -= 2) {
        var rtop = vranges[i]; var rbot = vranges[i + 1];
  ...
  function cursorCoords (cm, pos, context, lineObj, measurement) {
    ...
    function getBidi (ch, partPos) {
      var part = order[partPos]; var right = part.level % 2;
    ...
    var order = getOrder(lineObj); var ch = pos.ch;
  ...
  function coordsChar (cm, x, y) {
    ...
    var lineNo = lineAtHeight(doc, y); var last = doc.first + doc.size - 1;
  ...
  function coordsCharInner (cm, lineObj, lineNo, x, y) {
    ...
    var wrongLine = false; var adjust = 2 * cm.display.wrapper.clientWidth;
    ...
    var bidi = getOrder(lineObj); var dist = lineObj.text.length;
    var from = lineLeft(lineObj); var to = lineRight(lineObj);
    var fromX = getX(from); var fromOutside = wrongLine; var toX = getX(to); var toOutside = wrongLine;
    ...
    for (;;) {
      ...
      var step = Math.ceil(dist / 2); var middle = from + step;
      if (bidi) {
        middle = from;
  ...
  function endOperation (cm) {
    var op = cm.curOp; var doc = cm.doc; var display = cm.display;
    cm.curOp = null;
  ...
  function operation (cm1, f) {
    return function () {
      var cm = cm1 || this; var withOp = !cm.curOp;
  ...
  function docOperation (f) {
    return function () {
      var withOp = this.cm && !this.cm.curOp; var result;
  ...
  function runInOp (cm, f) {
    var withOp = !cm.curOp; var result;
    if (withOp) startOperation(cm);
  ...
  function fastPoll (cm) {
    ...
    function p () {
      ...
      if (!changed && !missed) { missed = true; cm.display.poll.set(60, p); } else { cm.display.pollingFast = false; slowPoll(cm); }
    }
  ...
  function readInput (cm) {
    var input = cm.display.input; var prevInput = cm.display.prevInput; var doc = cm.doc; var sel = doc.sel;
  ...
  function resetInput (cm, user) {
    var minimal; var selected; var doc = cm.doc;
  ...
  function posFromMouse (cm, e, liberal) {
    ...
    var x; var y; var space = getRect(display.lineSpace);
  ...
  function onMouseDown (e) {
    ...
    var cm = this; var display = cm.display; var doc = cm.doc; var sel = doc.sel;
    ...
    var now = +new Date(); var type = 'single';
    ...
    var startstart = sel.from; var startend = sel.to; var lastPos = start;
  ...
  function clickInGutter (cm, e) {
    var display = cm.display;
    try { var mX = e.clientX; var mY = e.clientY; } catch (e) { return false; }
  ...
  function onDrop (e) {
    ...
    if (files && files.length && window.FileReader && window.File) {
      ...
      try {
        ...
        if (text) {
          var curFrom = cm.doc.sel.from; var curTo = cm.doc.sel.to;
  ...
  function doHandleBinding (cm, bound, dropShift) {
    ...
    var doc = cm.doc; var prevShift = doc.sel.shift; var done = false;
    try {
  ...
  function handleKeyBinding (cm, e) {
    ...
    var startMap = getKeyMap(cm.options.keyMap); var next = startMap.auto;
    ...
    var name = keyName(e, true); var handled = false;
  ...
  function onKeyPress (e) {
    ...
    if (this.options.electricChars && this.doc.mode.electricChars &&
        ...
        this.doc.mode.electricChars.indexOf(ch) > -1) { setTimeout(operation(cm, function () { indentLine(cm, cm.doc.sel.to.line, 'smart'); }), 75); }
  ...
  function onContextMenu (cm, e) {
    ...
    var display = cm.display; var sel = cm.doc.sel;
    ...
    function rehide () {
      ...
      if (display.input.selectionStart != null) {
        ...
        var i = 0; var poll = function () {
          if (display.prevInput == ' ' && display.input.selectionStart == 0) { operation(cm, commands.selectAll)(cm); } else if (i++ < 10) detectingSelectAll = setTimeout(poll, 500);
          else resetInput(cm);
        };
  ...
  function clipPostChange (doc, change, pos) {
    ...
    if (pos.line > change.to.line + diff) {
      var preLine = pos.line - diff; var lastLine = doc.first + doc.size - 1;
  ...
  function skipAtomic (doc, pos, bias, mayClear) {
    var flipped = false; var curPos = pos;
    var dir = bias || 1;
    doc.cantEdit = false;
    search: for (;;) {
  ...
  function scrollCursorIntoView (cm) {
    ...
    if (!cm.state.focused) return;
    var display = cm.display; var box = getRect(display.sizer); var doScroll = null;
  ...
  function scrollPosIntoView (cm, pos, margin) {
    ...
    for (;;) {
      var changed = false; var coords = cursorCoords(cm, pos);
  ...
  function calculateScrollPos (cm, x1, y1, x2, y2) {
    ...
    if (y1 < 0) y1 = 0;
    var screen = display.scroller.clientHeight - scrollerCutOff; var screentop = display.scroller.scrollTop; var result = {};
    var docBottom = cm.doc.height + paddingVert(display);
    var atTop = y1 < snapMargin; var atBottom = y2 > docBottom - snapMargin;
    if (y1 < screentop) {
    ...
    var screenw = display.scroller.clientWidth - scrollerCutOff; var screenleft = display.scroller.scrollLeft;
  ...
  function changeLine (cm, handle, op) {
    var no = handle; var line = handle; var doc = cm.doc;
  ...
  function findPosH (doc, pos, dir, unit, visually) {
    var line = pos.line; var ch = pos.ch; var origDir = dir;
    var lineObj = getLine(doc, line);
    var possible = true;
  ...
  CodeMirror.prototype = {
    ...
    getTokenAt: function (pos, precise) {
      var doc = this.doc;
      pos = clipPos(doc, pos);
      var state = getStateBefore(this, pos.line, precise); var mode = this.doc.mode;
    ...
    heightAtLine: function (line, mode) {
      var end = false; var last = this.doc.first + this.doc.size - 1;
    ...
    clearGutter: operation(null, function (gutterID) {
      var cm = this; var doc = cm.doc; var i = doc.first;
      doc.iter(function (line) {
    ...
    getScrollInfo: function () {
      var scroller = this.display.scroller; var co = scrollerCutOff;
      return {
  ...
  option('readOnly', false, function (cm, val) {
    if (val == 'nocursor') { onBlur(cm); cm.display.input.blur(); } else if (!val) resetInput(cm, true);
  });
  ...
  var modes = CodeMirror.modes = {}; var mimeModes = CodeMirror.mimeModes = {};
  ...
  var commands = CodeMirror.commands = {
    ...
    goLineStartSmart: function (cm) {
      var cur = cm.getCursor(); var start = lineStart(cm, cur.line);
  ...
  CodeMirror.fromTextArea = function (textarea, options) {
    ...
    if (textarea.form) {
      ...
      if (!options.leaveSubmitMethodAlone) {
        var form = textarea.form; var realSubmit = form.submit;
        try {
  ...
  TextMarker.prototype.clear = function () {
    ...
    if (cm && this.collapsed && !cm.options.lineWrapping) {
      for (var i = 0; i < this.lines.length; ++i) {
        var visual = visualLine(cm.doc, this.lines[i]); var len = lineLength(cm.doc, visual);
  ...
  TextMarker.prototype.changed = function () {
    var pos = this.find(); var cm = this.doc.cm;
    if (!pos || !cm) return;
  ...
  function markText (doc, from, to, options, type) {
    ...
    var curLine = from.line; var size = 0; var collapsedAtStart; var collapsedAtEnd; var cm = doc.cm; var updateMaxLine;
  ...
  function stretchSpansOverChange (doc, change) {
    ...
    var startCh = change.from.ch; var endCh = change.to.ch; var isInsert = posEq(change.from, change.to);
    ...
    if (!sameLine) {
      ...
      if (gap > 0 && first) {
        for (var i = 0; i < first.length; ++i) {
          if (first[i].to == null) { (gapMarkers || (gapMarkers = [])).push({ from: null, to: null, marker: first[i].marker }); }
        }
      }
  ...
  function removeReadOnlyRanges (doc, from, to) {
    ...
    for (var i = 0; i < markers.length; ++i) {
      var mk = markers[i]; var m = mk.find();
    ...
    if (sps) {
      for (var sp, i = 0; i < sps.length; ++i) {
        ...
        if ((sp.from == null || sp.from < ch) &&
          (sp.to == null || sp.to > ch) &&
          (!found || found.width < sp.marker.width)) { found = sp.marker; }
      }
    }
    return found;
  }
  ...
  function lineIsHiddenInner (doc, line, span) {
    if (span.to == null) {
      var end = span.marker.find().to; var endLine = getLine(doc, end.line);
  ...
  LineWidget.prototype.clear = widgetOperation(function () {
    var ws = this.line.widgets; var no = lineNo(this.line);
  ...
  function runMode (cm, text, mode, state, f) {
    ...
    var stream = new StringStream(text, cm.options.tabSize); var style;
  ...
  function lineContent (cm, realLine, measure, copyWidgets) {
    var merged; var line = realLine; var empty = true;
    ...
    if (measure && ie && (order = getOrder(line))) {
      ...
      var last = order[l]; var prev = order[l - 1];
  ...
  function buildToken (builder, text, style, startStyle, endStyle, title) {
    ...
    if (!tokenSpecialChars.test(text)) {
      ...
      var content = document.createDocumentFragment(); var pos = 0;
      while (true) {
  ...
  function buildTokenMeasure (builder, text, style, startStyle, endStyle) {
    ...
    for (var i = 0; i < text.length; ++i) {
      var ch = text.charAt(i); var start = i == 0;
      ...
      if (ie && wrapping && ch == ' ' && i && !/\s/.test(text.charAt(i - 1)) &&
          i < text.length - 1 && !/\s/.test(text.charAt(i + 1))) { span.style.whiteSpace = 'normal'; }
    ...
    if (widget) {
      ...
      if (builder.measure) {
        if (size) {
          ...
          if (marker.type != 'bookmark' || marker.insertLeft) { builder.pre.insertBefore(elt, widget); } else { builder.pre.appendChild(elt); }
        }
        builder.measuredSomething = true;
      }
    }
  ...
  function insertLineContent (line, builder, styles) {
    ...
    var len = allText.length; var pos = 0; var i = 1; var text = ''; var style;
    var nextChange = 0; var spanStyle; var spanEndStyle; var spanStartStyle; var title; var collapsed;
    for (;;) {
  ...
  function addToHistory (doc, change, selAfter, opId) {
    ...
    var time = +new Date(); var cur = lst(hist.done);
  ...
  function copyHistoryArray (events, newGroup) {
    for (var i = 0, copy = []; i < events.length; ++i) {
      var event = events[i]; var changes = event.changes; var newChanges = [];
      copy.push({
        changes: newChanges,
  ...
  var delayedCallbacks; var delayedCallbackDepth = 0;
  ...
  function moveVisually (line, start, dir, byUnit) {
    ...
    var pos = getBidiPartAt(bidi, start); var part = bidi[pos];
  ...
  var bidiOrdering = (function () {
    ...
    function charType (code) {
      ...
      else if (code >= 0x600 && code <= 0x6ff) return arabicTypes.charAt(code - 0x600);
...
CodeMirror.defineMode('markdown', function (cmCfg, modeCfg) {
  ...
  var getMode = (function () {
    var i; var modes = {}; var mimes = {}; var mime;
  ...
  function getType (state) {
    ...
    if (state.strong) { styles.push(state.em ? emstrong : strong); } else if (state.em) { styles.push(em); }
...
// Showdown namespace
//
var Showdown = { extensions:{} }; var forEach = Showdown.forEach = function (a, b) { if (typeof a.forEach === 'function')a.forEach(b); else { var c; var d = a.length; for (c = 0; c < d; c++)b(a[c], c, a) } }; var  stdExtName = function (a) { return a.replace(/[_-]||\s/g, '').toLowerCase() }; Showdown.converter = function (a) { var b; var c; var d; var e = 0; var f = []; var  g = []; if (typeof module !== 'undefind' && typeof exports !== 'undefined' && typeof require !== 'undefind') { var h = require('fs'); if (h) { var i = h.readdirSync((__dirname || '.') + '/extensions').filter(function (a) { return ~a.indexOf('.js') }).map(function (a) { return a.replace(/\.js$/, '') }); Showdown.forEach(i, function (a) { var b = stdExtName(a); Showdown.extensions[b] = require('./extensions/' + a) }) } } this.makeHtml = function (a) { return b = {}, c = {}, d = [], a = a.replace(/~/g, '~T'), a = a.replace(/\$/g, '~D'), a = a.replace(/\r\n/g, '\n'), a = a.replace(/\r/g, '\n'), a = '\n\n' + a + '\n\n', a = M(a), a = a.replace(/^[ \t]+$/mg, ''), Showdown.forEach(f, function (b) { a = k(b, a) }), a = z(a), a = m(a), a = l(a), a = o(a), a = K(a), a = a.replace(/~D/g, '$$'), a = a.replace(/~T/g, '~'), Showdown.forEach(g, function (b) { a = k(b, a) }), a }; if (a && a.extensions) { var j = this; Showdown.forEach(a.extensions, function (a) { typeof a === 'string' && (a = Showdown.extensions[stdExtName(a)]); if (typeof a !== 'function') throw 'Extension \'' + a + '\' could not be loaded.  It was either not found or is not a valid extension.'; Showdown.forEach(a(j), function (a) { a.type ? a.type === 'language' || a.type === 'lang' ? f.push(a) : (a.type === 'output' || a.type === 'html') && g.push(a) : g.push(a) }) }) } var k = function (a, b) { if (a.regex) { var c = new RegExp(a.regex, 'g'); return b.replace(c, a.replace) } if (a.filter) return a.filter(b) }; var l = function (a) { return a += '~0', a = a.replace(/^[ ]{0,3}\[(.+)\]:[ \t]*\n?[ \t]*<?(\S+?)>?[ \t]*\n?[ \t]*(?:(\n*)["(](.+?)[")][ \t]*)?(?:\n+|(?=~0))/gm, function (a, d, e, f, g) { return d = d.toLowerCase(), b[d] = G(e), f ? f + g : (g && (c[d] = g.replace(/"/g, '&quot;')), '') }), a = a.replace(/~0/, ''), a }; var m = function (a) { a = a.replace(/\n/g, '\n\n'); var b = 'p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|script|noscript|form|fieldset|iframe|math|ins|del|style|section|header|footer|nav|article|aside'; var c = 'p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|script|noscript|form|fieldset|iframe|math|style|section|header|footer|nav|article|aside'; return a = a.replace(/^(<(p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|script|noscript|form|fieldset|iframe|math|ins|del)\b[^\r]*?\n<\/\2>[ \t]*(?=\n+))/gm, n), a = a.replace(/^(<(p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|script|noscript|form|fieldset|iframe|math|style|section|header|footer|nav|article|aside)\b[^\r]*?<\/\2>[ \t]*(?=\n+)\n)/gm, n), a = a.replace(/(\n[ ]{0,3}(<(hr)\b([^<>])*?\/?>)[ \t]*(?=\n{2,}))/g, n), a = a.replace(/(\n\n[ ]{0,3}<!(--[^\r]*?--\s*)+>[ \t]*(?=\n{2,}))/g, n), a = a.replace(/(?:\n\n)([ ]{0,3}(?:<([?%])[^\r]*?\2>)[ \t]*(?=\n{2,}))/g, n), a = a.replace(/\n\n/g, '\n'), a }; var n = function (a, b) { var c = b; return c = c.replace(/\n\n/g, '\n'), c = c.replace(/^\n/, ''), c = c.replace(/\n+$/g, ''), c = '\n\n~K' + (d.push(c) - 1) + 'K\n\n', c }; var o = function (a) { a = v(a); var b = A('<hr />'); return a = a.replace(/^[ ]{0,2}([ ]?\*[ ]?){3,}[ \t]*$/gm, b), a = a.replace(/^[ ]{0,2}([ ]?\-[ ]?){3,}[ \t]*$/gm, b), a = a.replace(/^[ ]{0,2}([ ]?\_[ ]?){3,}[ \t]*$/gm, b), a = x(a), a = y(a), a = E(a), a = m(a), a = F(a), a }; var p = function (a) { return a = B(a), a = q(a), a = H(a), a = t(a), a = r(a), a = I(a), a = G(a), a = D(a), a = a.replace(/  +\n/g, ' <br />\n'), a }; var q = function (a) { var b = /(<[a-z\/!$]("[^"]*"|'[^']*'|[^'">])*>|<!(--.*?--\s*)+>)/gi; return a = a.replace(b, function (a) { var b = a.replace(/(.)<\/?code>(?=.)/g, '$1`'); return b = N(b, '\\`*_'), b }), a }; var r = function (a) { return a = a.replace(/(\[((?:\[[^\]]*\]|[^\[\]])*)\][ ]?(?:\n[ ]*)?\[(.*?)\])()()()()/g, s), a = a.replace(/(\[((?:\[[^\]]*\]|[^\[\]])*)\]\([ \t]*()<?(.*?(?:\(.*?\).*?)?)>?[ \t]*((['"])(.*?)\6[ \t]*)?\))/g, s), a = a.replace(/(\[([^\[\]]+)\])()()()()()/g, s), a }; var s = function (a, d, e, f, g, h, i, j) { j == undefined && (j = ''); var k = d; var l = e; var m = f.toLowerCase(); var n = g; var o = j; if (n == '') { m == '' && (m = l.toLowerCase().replace(/ ?\n/g, ' ')), n = '#' + m; if (b[m] != undefined)n = b[m], c[m] != undefined && (o = c[m]); else { if (!(k.search(/\(\s*\)$/m) > -1)) return k; n = '' } }n = N(n, '*_'); var p = '<a href="' + n + '"'; return o != '' && (o = o.replace(/"/g, '&quot;'), o = N(o, '*_'), p += ' title="' + o + '"'), p += '>' + l + '</a>', p }; var t = function (a) { return a = a.replace(/(!\[(.*?)\][ ]?(?:\n[ ]*)?\[(.*?)\])()()()()/g, u), a = a.replace(/(!\[(.*?)\]\s?\([ \t]*()<?(\S+?)>?[ \t]*((['"])(.*?)\6[ \t]*)?\))/g, u), a }; var u = function (a, d, e, f, g, h, i, j) { var k = d; var l = e; var m = f.toLowerCase(); var n = g; var o = j; o || (o = ''); if (n == '') { m == '' && (m = l.toLowerCase().replace(/ ?\n/g, ' ')), n = '#' + m; if (b[m] == undefined) return k; n = b[m], c[m] != undefined && (o = c[m]) }l = l.replace(/"/g, '&quot;'), n = N(n, '*_'); var p = '<img src="' + n + '" alt="' + l + '"'; return o = o.replace(/"/g, '&quot;'), o = N(o, '*_'), p += ' title="' + o + '"', p += ' />', p }; var v = function (a) { function b (a) { return a.replace(/[^\w]/g, '').toLowerCase() } return a = a.replace(/^(.+)[ \t]*\n=+[ \t]*\n+/gm, function (a, c) { return A('<h1 id="' + b(c) + '">' + p(c) + '</h1>') }), a = a.replace(/^(.+)[ \t]*\n-+[ \t]*\n+/gm, function (a, c) { return A('<h2 id="' + b(c) + '">' + p(c) + '</h2>') }), a = a.replace(/^(\#{1,6})[ \t]*(.+?)[ \t]*\#*\n+/gm, function (a, c, d) { var e = c.length; return A('<h' + e + ' id="' + b(d) + '">' + p(d) + '</h' + e + '>') }), a }; var w; var  x = function (a) { a += '~0'; var b = /^(([ ]{0,3}([*+-]|\d+[.])[ \t]+)[^\r]+?(~0|\n{2,}(?=\S)(?![ \t]*(?:[*+-]|\d+[.])[ \t]+)))/gm; return e ? a = a.replace(b, function (a, b, c) { var d = b; var  e = c.search(/[*+-]/g) > -1 ? 'ul' : 'ol'; d = d.replace(/\n{2,}/g, '\n\n\n'); var f = w(d); return f = f.replace(/\s+$/, ''), f = '<' + e + '>' + f + '</' + e + '>\n', f }) : (b = /(\n\n|^\n?)(([ ]{0,3}([*+-]|\d+[.])[ \t]+)[^\r]+?(~0|\n{2,}(?=\S)(?![ \t]*(?:[*+-]|\d+[.])[ \t]+)))/g, a = a.replace(b, function (a, b, c, d) { var e = b; var f = c; var g = d.search(/[*+-]/g) > -1 ? 'ul' : 'ol'; var f = f.replace(/\n{2,}/g, '\n\n\n'); var  h = w(f); return h = e + '<' + g + '>\n' + h + '</' + g + '>\n', h })), a = a.replace(/~0/, ''), a }; w = function (a) { return e++, a = a.replace(/\n{2,}$/, '\n'), a += '~0', a = a.replace(/(\n)?(^[ \t]*)([*+-]|\d+[.])[ \t]+([^\r]+?(\n{1,2}))(?=\n*(~0|\2([*+-]|\d+[.])[ \t]+))/gm, function (a, b, c, d, e) { var f = e; var g = b; var  h = c; return g || f.search(/\n{2,}/) > -1 ? f = o(L(f)) : (f = x(L(f)), f = f.replace(/\n$/, ''), f = p(f)), '<li>' + f + '</li>\n' }), a = a.replace(/~0/g, ''), e--, a }; var y = function (a) { return a += '~0', a = a.replace(/(?:\n\n|^)((?:(?:[ ]{4}|\t).*\n+)+)(\n*[ ]{0,3}[^ \t\n]|(?=~0))/g, function (a, b, c) { var d = b; var  e = c; return d = C(L(d)), d = M(d), d = d.replace(/^\n+/g, ''), d = d.replace(/\n+$/g, ''), d = '<pre><code>' + d + '\n</code></pre>', A(d) + e }), a = a.replace(/~0/, ''), a }; var z = function (a) { return a += '~0', a = a.replace(/(?:^|\n)```(.*)\n([\s\S]*?)\n```/g, function (a, b, c) { var d = b; var e = c; return e = C(e), e = M(e), e = e.replace(/^\n+/g, ''), e = e.replace(/\n+$/g, ''), e = '<pre><code' + (d ? ' class="' + d + '"' : '') + '>' + e + '\n</code></pre>', A(e) }), a = a.replace(/~0/, ''), a }; var A = function (a) { return a = a.replace(/(^\n+|\n+$)/g, ''), '\n\n~K' + (d.push(a) - 1) + 'K\n\n' }; var B = function (a) { return a = a.replace(/(^|[^\\])(`+)([^\r]*?[^`])\2(?!`)/gm, function (a, b, c, d, e) { var f = d; return f = f.replace(/^([ \t]*)/g, ''), f = f.replace(/[ \t]*$/g, ''), f = C(f), b + '<code>' + f + '</code>' }), a }; var C = function (a) { return a = a.replace(/&/g, '&amp;'), a = a.replace(/</g, '&lt;'), a = a.replace(/>/g, '&gt;'), a = N(a, '*_{}[]\\', !1), a }; var D = function (a) { return a = a.replace(/(\*\*|__)(?=\S)([^\r]*?\S[*_]*)\1/g, '<strong>$2</strong>'), a = a.replace(/(\*|_)(?=\S)([^\r]*?\S)\1/g, '<em>$2</em>'), a }; var E = function (a) { return a = a.replace(/((^[ \t]*>[ \t]?.+\n(.+\n)*\n*)+)/gm, function (a, b) { var c = b; return c = c.replace(/^[ \t]*>[ \t]?/gm, '~0'), c = c.replace(/~0/g, ''), c = c.replace(/^[ \t]+$/gm, ''), c = o(c), c = c.replace(/(^|\n)/g, '$1  '), c = c.replace(/(\s*<pre>[^\r]+?<\/pre>)/gm, function (a, b) { var c = b; return c = c.replace(/^ {2}/mg, '~0'), c = c.replace(/~0/g, ''), c }), A('<blockquote>\n' + c + '\n</blockquote>') }), a }; var F = function (a) { a = a.replace(/^\n+/g, ''), a = a.replace(/\n+$/g, ''); var b = a.split(/\n{2,}/g); var c = []; var e = b.length; for (var f = 0; f < e; f++) { var g = b[f]; g.search(/~K(\d+)K/g) >= 0 ? c.push(g) : g.search(/\S/) >= 0 && (g = p(g), g = g.replace(/^([ \t]*)/g, '<p>'), g += '</p>', c.push(g)) }e = c.length; for (var f = 0; f < e; f++) while (c[f].search(/~K(\d+)K/) >= 0) { var h = d[RegExp.$1]; h = h.replace(/\$/g, '$$$$'), c[f] = c[f].replace(/~K\d+K/, h) } return c.join('\n\n') }; var G = function (a) { return a = a.replace(/&(?!#?[xX]?(?:[0-9a-fA-F]+|\w+);)/g, '&amp;'), a = a.replace(/<(?![a-z\/?\$!])/gi, '&lt;'), a }; var H = function (a) { return a = a.replace(/\\(\\)/g, O), a = a.replace(/\\([`*_{}\[\]()>#+-.!])/g, O), a }; var I = function (a) { return a = a.replace(/<((https?|ftp|dict):[^'">\s]+)>/gi, '<a href="$1">$1</a>'), a = a.replace(/<(?:mailto:)?([-.\w]+\@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)>/gi, function (a, b) { return J(K(b)) }), a }; var J = function (a) { var b = [function (a) { return '&#' + a.charCodeAt(0) + ';' }, function (a) { return '&#x' + a.charCodeAt(0).toString(16) + ';' }, function (a) { return a }]; return a = 'mailto:' + a, a = a.replace(/./g, function (a) { if (a == '@')a = b[Math.floor(Math.random() * 2)](a); else if (a != ':') { var c = Math.random(); a = c > 0.9 ? b[2](a) : c > 0.45 ? b[1](a) : b[0](a) } return a }), a = '<a href="' + a + '">' + a + '</a>', a = a.replace(/">.+:/g, '">'), a }; var K = function (a) { return a = a.replace(/~E(\d+)E/g, function (a, b) { var c = parseInt(b); return String.fromCharCode(c) }), a }; var L = function (a) { return a = a.replace(/^(\t|[ ]{1,4})/gm, '~0'), a = a.replace(/~0/g, ''), a }; var M = function (a) { return a = a.replace(/\t(?=\t)/g, '    '), a = a.replace(/\t/g, '~A~B'), a = a.replace(/~B(.+?)~A/g, function (a, b, c) { var d = b; var e = 4 - d.length % 4; for (var f = 0; f < e; f++)d += ' '; return d }), a = a.replace(/~A/g, '    '), a = a.replace(/~B/g, ''), a }; var N = function (a, b, c) { var d = '([' + b.replace(/([\[\]\\])/g, '\\$1') + '])'; c && (d = '\\\\' + d); var e = new RegExp(d, 'g'); return a = a.replace(e, O), a }; var  O = function (a, b) { var c = b.charCodeAt(0); return '~E' + c + 'E' } }, typeof module !== 'undefined' && (module.exports = Showdown), typeof define === 'function' && define.amd && define('showdown', function () { return Showdown });
  ...
  $(function () {
    ...
    $(document).ready(function () {
      ...
      function syncScroll (e) {
        ...
        var $previewViewport = $('.entry-preview-content');
        var $codeContent = $('.CodeMirror-sizer');
        var $previewContent = $('.rendered-markdown');
